...
const HeroNews: React.FC<HeroNewsProps> = ({
    ...
    return (
        <header className="relative overflow-hidden bg-white pt-28 pb-12">
            {/* ── Decorative background ─────────────────────────── */}
            <div aria-hidden="true" className="pointer-events-none absolute inset-0">
                <div
                    className="absolute -top-40 -right-40 h-[500px] w-[500px] rounded-full opacity-[0.07] blur-3xl"
            ...
            <div className="relative container mx-auto px-4">
                {/* ── Edition meta bar ──────────────────────────────── */}
                <div className="mb-8 flex items-center justify-between border-y border-gray-100 py-2.5">
                ...
                <div className="mx-auto max-w-4xl">
                    ...
                    <h1 className="mb-2 text-center text-6xl font-black leading-none tracking-tighter text-gray-900 md:text-8xl lg:text-[9rem]">
                        Beri
                        <span
                            className="bg-clip-text text-transparent"
                            style={{ backgroundImage: 'linear-gradient(135deg, #2ECC71 0%, #27ae60 100%)' }}
                        >
                            ta
                        </span>
                    </h1>
                    ...
                    {/* School sub-line */}
                    <p className="mb-10 text-center text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                        MI NU 02 Situwangi &mdash; Berita &amp; Informasi Sekolah
                    </p>

                    {/* ── Search bar ────────────────────────────────── */}
                    <div className="mx-auto max-w-2xl">
                        <div className="flex items-center rounded-2xl bg-white shadow-lg shadow-gray-100/80 ring-1 ring-gray-200 transition-shadow focus-within:shadow-emerald-100 focus-within:ring-2 focus-within:ring-[#2ECC71]/50">
                            ...
                            <input
                                ...
                                placeholder="Cari berita, kegiatan, pengumuman…"
                                className="flex-1 bg-transparent py-4 pl-3 pr-2 text-sm text-gray-800 placeholder-gray-400 focus:outline-none"
                                aria-label="Cari berita"
                            />
                            <button
                                onClick={onSearchSubmit}
                                className="m-1.5 rounded-xl px-5 py-2.5 text-sm font-bold text-white transition-all hover:opacity-90 active:scale-95"
                        ...
                        {/* {search === '' && (
                            <p className="mt-3 text-center text-xs text-gray-400">
                                Coba:{' '}
                                {['kegiatan', 'prestasi', 'pengumuman'].map((hint, i) => (
                                    <span key={hint}>
                                        {i > 0 && <span className="mx-1 text-gray-200">·</span>}
                                        <button
                                            onClick={() => onSearchChange(hint)}
                                            className="font-semibold text-[#27ae60] underline-offset-2 hover:underline"
                                        >
                                            {hint}
                                        </button>
                                    </span>
